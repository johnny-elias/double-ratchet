...
struct DoubleRatchetHeader {
    ...
    std::string serialize() const {
        ...
        uint32_t pub_key_size_net = Util::hton(dh_pub.size()); // Size in network byte order
        s.append(reinterpret_cast<const char*>(&pub_key_size_net), sizeof(pub_key_size_net));
        s.append(reinterpret_cast<const char*>(dh_pub.BytePtr()), dh_pub.size());

        uint32_t pn_net = Util::hton(pn); // Ensure network byte order
        s.append(reinterpret_cast<const char*>(&pn_net), sizeof(pn_net));

        uint32_t n_net = Util::hton(n); // Ensure network byte order
        s.append(reinterpret_cast<const char*>(&n_net), sizeof(n_net));
        return s;
    }
    ...
    size_t deserialize(const std::string& data) {
        ...
        uint32_t pub_key_size_net;
        std::memcpy(&pub_key_size_net, data.data() + offset, sizeof(pub_key_size_net));
        uint32_t pub_key_size = Util::ntoh(pub_key_size_net); // Convert from network byte order
        offset += sizeof(pub_key_size_net);
        ...
        if (data.size() < offset + sizeof(uint32_t)) return 0;
        uint32_t pn_net;
        std::memcpy(&pn_net, data.data() + offset, sizeof(pn_net));
        pn = Util::ntoh(pn_net); // Convert back from network byte order
        offset += sizeof(pn_net);

        // Read N
        if (data.size() < offset + sizeof(uint32_t)) return 0;
        uint32_t n_net;
        std::memcpy(&n_net, data.data() + offset, sizeof(n_net));
        n = Util::ntoh(n_net); // Convert back from network byte order
        offset += sizeof(n_net);
...
struct Message_KeyExchange : public Message {
  ...
  std::string serialize() const override {
      ...
      uint32_t iv_size_net = Util::hton(iv.size());
      serialized_data.append(reinterpret_cast<const char*>(&iv_size_net), sizeof(iv_size_net));
      serialized_data.append(reinterpret_cast<const char*>(iv.BytePtr()), iv.size());

      uint32_t pub_val_size_net = Util::hton(pub_val.size());
      serialized_data.append(reinterpret_cast<const char*>(&pub_val_size_net), sizeof(pub_val_size_net));
      serialized_data.append(reinterpret_cast<const char*>(pub_val.BytePtr()), pub_val.size());
  ...
  size_t deserialize(const std::string &bytes) override {
      ...
      if (bytes.size() < offset + sizeof(iv_size_net)) throw std::runtime_error("Deserialization error: IV size missing");
      std::memcpy(&iv_size_net, bytes.data() + offset, sizeof(iv_size_net));
      uint32_t iv_size = Util::ntoh(iv_size_net);
      offset += sizeof(iv_size_net);
      ...
      uint32_t pub_val_size_net;
       if (bytes.size() < offset + sizeof(pub_val_size_net)) throw std::runtime_error("Deserialization error: pub_val size missing");
      std::memcpy(&pub_val_size_net, bytes.data() + offset, sizeof(pub_val_size_net));
      uint32_t pub_val_size = Util::ntoh(pub_val_size_net);
      offset += sizeof(pub_val_size_net);
...
struct Message_Message : public Message {
  ...
  std::string serialize() const override {
      ...
      uint32_t header_size_net = Util::hton(serialized_header.size());
      serialized_data.append(reinterpret_cast<const char*>(&header_size_net), sizeof(header_size_net));
      ...
      uint32_t ct_size_net = Util::hton(ciphertext.size());
      serialized_data.append(reinterpret_cast<const char*>(&ct_size_net), sizeof(ct_size_net));
      serialized_data.append(reinterpret_cast<const char*>(ciphertext.BytePtr()), ciphertext.size());

       // Serialize MAC
      uint32_t mac_size_net = Util::hton(mac.size());
      serialized_data.append(reinterpret_cast<const char*>(&mac_size_net), sizeof(mac_size_net));
      serialized_data.append(reinterpret_cast<const char*>(mac.BytePtr()), mac.size());
  ...
  size_t deserialize(const std::string &bytes) override {
      ...
      if (bytes.size() < offset + sizeof(header_size_net)) throw std::runtime_error("Deserialization error: header size missing");
      std::memcpy(&header_size_net, bytes.data() + offset, sizeof(header_size_net));
      uint32_t header_size = Util::ntoh(header_size_net);
      offset += sizeof(header_size_net);
      ...
      uint32_t ct_size_net;
      if (bytes.size() < offset + sizeof(ct_size_net)) throw std::runtime_error("Deserialization error: ciphertext size missing");
      std::memcpy(&ct_size_net, bytes.data() + offset, sizeof(ct_size_net));
      uint32_t ct_size = Util::ntoh(ct_size_net);
      offset += sizeof(ct_size_net);
      ...
      offset += ct_size;
       ...
       // Deserialize MAC
      uint32_t mac_size_net;
       if (bytes.size() < offset + sizeof(mac_size_net)) throw std::runtime_error("Deserialization error: MAC size missing");
      std::memcpy(&mac_size_net, bytes.data() + offset, sizeof(mac_size_net));
      uint32_t mac_size = Util::ntoh(mac_size_net);
      offset += sizeof(mac_size_net);
